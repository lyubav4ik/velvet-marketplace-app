...
Зарегистрировано: `vl-maison-header` (id=2) — шапка MAISON v6: пункты меню распределяются по ширине (flex-grow слоты, column-gap 20px, перенос на 2-ю строку), лого-картинка рядом с иконками справа, критичная раскладка продублирована инлайн-стилями (работает в редакторе), ассеты на теге v0.6, поиск ведёт в корень каталога (/katalog/, /catalog/, /shop/ — автоопределение).
