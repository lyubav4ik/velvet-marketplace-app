...
Зарегистрировано: `vl-maison-header` (id=2, тег v0.7) — шапка MAISON: одна строка [меню | лого | иконки], критичная раскладка инлайнами, панель «Дизайн» заполнена, поиск ведёт в корень каталога с `?q=` (каталог сам фильтрует как штатный блок). Живой блок пересоздан на новом манифесте: подпись пункта меню «Пункт меню», названия карточек = тексты пунктов, якорь b337 сохранён.
