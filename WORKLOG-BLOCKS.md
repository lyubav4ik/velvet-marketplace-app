...
Зарегистрировано: `vl-maison-header` (id=2, тег v0.8) — шапка MAISON: одна строка [меню | лого | иконки], критичная раскладка инлайнами, поиск ведёт в корень каталога (`?q=`), карточки-пункты названы по тексту, у трёх иконок (кабинет/корзина/поиск) — выбор иконки + цвет + размер, фон блока управляется из панели.
